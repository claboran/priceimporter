...
alter table int_test_d_region add CONSTRAINT int_test_region_un UNIQUE (region);
...
alter table int_test_d_period_type add CONSTRAINT int_test_period_un UNIQUE (period_type);
...
alter table int_test_d_date_time add CONSTRAINT int_test_date_time_un UNIQUE (day_of_month, month_of_year, the_year, hour_of_day, minutes_of_hour);
...
-- Spring Batch tables
--
CREATE TABLE INT_TEST_BATCH_JOB_INSTANCE  (
  JOB_INSTANCE_ID BIGINT IDENTITY NOT NULL PRIMARY KEY ,
  VERSION BIGINT ,
  JOB_NAME VARCHAR(100) NOT NULL,
  JOB_KEY VARCHAR(32) NOT NULL,
  constraint INT_TEST_JOB_INST_UN unique (JOB_NAME, JOB_KEY)
) ;

CREATE TABLE INT_TEST_BATCH_JOB_EXECUTION  (
  ...
  constraint INT_TEST_JOB_INST_EXEC_FK foreign key (JOB_INSTANCE_ID)
  references INT_TEST_BATCH_JOB_INSTANCE(JOB_INSTANCE_ID)
) ;

CREATE TABLE INT_TEST_BATCH_JOB_EXECUTION_PARAMS  (
  ...
  constraint INT_TEST_JOB_EXEC_PARAMS_FK foreign key (JOB_EXECUTION_ID)
  references INT_TEST_BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
) ;

CREATE TABLE INT_TEST_BATCH_STEP_EXECUTION  (
  ...
  EXIT_MESSAGE VARCHAR(2500) ,
  LAST_UPDATED TIMESTAMP,
  constraint INT_TEST_JOB_EXEC_STEP_FK foreign key (JOB_EXECUTION_ID)
  references INT_TEST_BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
) ;

CREATE TABLE INT_TEST_BATCH_STEP_EXECUTION_CONTEXT  (
  STEP_EXECUTION_ID BIGINT NOT NULL PRIMARY KEY,
  SHORT_CONTEXT VARCHAR(2500) NOT NULL,
  SERIALIZED_CONTEXT LONGVARCHAR ,
  constraint INT_TEST_STEP_EXEC_CTX_FK foreign key (STEP_EXECUTION_ID)
  references INT_TEST_BATCH_STEP_EXECUTION(STEP_EXECUTION_ID)
) ;

CREATE TABLE INT_TEST_BATCH_JOB_EXECUTION_CONTEXT  (
  JOB_EXECUTION_ID BIGINT NOT NULL PRIMARY KEY,
  SHORT_CONTEXT VARCHAR(2500) NOT NULL,
  SERIALIZED_CONTEXT LONGVARCHAR ,
  constraint INT_TEST_JOB_EXEC_CTX_FK foreign key (JOB_EXECUTION_ID)
  references INT_TEST_BATCH_JOB_EXECUTION(JOB_EXECUTION_ID)
) ;
